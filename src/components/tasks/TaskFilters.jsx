        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none max-w-full">
            {FILTER_OPTIONS.map((opt) => (
                <button
                    ...
                    className={cn(
                        "shrink-0 rounded-xl px-3 py-1.5 text-xs font-semibold transition-all active:scale-95 cursor-pointer",
                        activeId === opt.id
                            ? "bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 shadow-2xs"
                            : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200/80 dark:bg-zinc-800/80 dark:text-zinc-400 dark:hover:bg-zinc-700/80"
                    )}
                >
                    {opt.label}
                </button>
